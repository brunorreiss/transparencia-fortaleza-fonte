...
async def process_detalhes(soup: BeautifulSoup) -> list[Receita]:
    ...
    for row in rows[1:]:
        ...
        # Criando o dicionário de dados para a receita
        receita = {headers[i]: float(cell.text.strip().replace('.', '').replace(',', '.')) if i in [2, 3, 4, 5] else cell.text.strip() for i, cell in enumerate(cells)}
        
        # Cria uma instância de Receita usando ** para desempacotar o dicionário como argumentos de palavra-chave
...
async def fetch(data_inicio, data_fim, ano_exercicio):
    ...
    try:
        ...
        if resp.status != 200:
            ...
            # Colocando data_inicio e data_fim no resultado
